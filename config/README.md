...
| Preferred microphone | `voice.py` `MIC_PREFERENCE` | **next time a mic is resolved** — the Microphone card's button, a hot-plug, or a restart. The one knob that is not immediate, which is why it lives on that card rather than in the Settings tab |
